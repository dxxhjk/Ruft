...
pub struct AppendEntriesRequest {
    ...
    pub leader_client_addr: SocketAddr,
...
impl Redirect {
    pub fn new(command: String, src: SocketAddr) -> Redirect {
        Redirect { command, src }
    }
}
    ...
    pub fn start_listener(&self, rpc_notifier: Sender<RPCMessage>) -> Result<(), Box<dyn Error>> {
        loop {
            ...
            let (amt, _) = match self.socket.recv_from(&mut buffer) {
                Ok(pair) => pair,
                Err(_) => {
                    error!("{} receive RPC error", self.socket_addr.port());
